...
export const registerSocketHandler = (socket: Socket, io: Server) => {
    ...
    socket.on("lock-seats", async ({ showId, seatIds, userId }) => {
        ...
        try {
             ...
             */
            const seatLockKeys = seatIds.map((seatId: string) =>`seat-lock:${showId}:${seatId}`);
            ...
            if (unavailableSeats.length > 0) {
                await redis.unwatch();
                socket.emit("seat-locked-failed", {showId,requested: seatIds,alreadyLocked: unavailableSeats,});
                return;
            }
            ...
            for (const seatId of seatIds) {
                ...
                transaction.set(seatLockKey,userId,"EX",300,"NX");
            }
            ...
            if (result === null) {
                socket.emit("seat-locked-failed", {showId,requested: seatIds,alreadyLocked: [],message:"Seats were locked by another user. Please try again.",});
                return;
            }
            /*
             ...
             */
            const lockSuccessful = result.every(([error, value]) => !error && value === "OK");
            if (!lockSuccessful) {
                socket.emit("seat-locked-failed",{showId,requested: seatIds, alreadyLocked: [], message:"Some seats could not be locked. Please try again.",});
                return;
            }
            // Notify all users in the room about updated locked seats
            io.to(showId).emit("seats-locked", {showId,seatIds,userId,});
            /*
             * IMPORTANT:
             *
            ...
            socket.emit("seat-lock-success", {showId, seatIds, });
        } catch (error) {
            try {
                await redis.unwatch();
            } catch (unwatchError) {
            ...
            socket.emit("seat-locked-failed", {showId,requested: seatIds,alreadyLocked: [], message:"Something went wrong while locking seats.",});
    }
    });
