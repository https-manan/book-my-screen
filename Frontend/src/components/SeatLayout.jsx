...
const SeatLayout = () => {
    ...
    useEffect(() => {
        ...
        const handleSeatsLocked = ({ seatIds }) => {
            setLockedSeats((prev) => Array.from(new Set([...prev,...seatIds])));
        };

        const handleSeatsUnlocked = ({ seatIds }) => {
            setLockedSeats((prev) =>prev.filter((seatId) =>!seatIds.includes(seatId)));
        };
        socket.on(
    ...
    useEffect(() => {
        const handleLockSuccess = ({showId: lockedShowId}) => {
            ...
            navigate( `/show/${showId}/${data?.show?.location}/checkout`,{state: {selectedSeats,totalPrice}});};

        const handleLockFailed = ({showId: failedShowId,alreadyLocked,message }) => {
            if (failedShowId !== showId) return;
            setIsLocking(false);
            if (alreadyLocked?.length > 0) {
                alert(`These seats are already locked: ${alreadyLocked.join(", ")}`);
            } else {
                alert(message || "Seats could not be locked. Please try again.");
            }
        };


        socket.on( "seat-lock-success",handleLockSuccess);
        socket.on("seat-locked-failed", handleLockFailed);

        return () => {
            socket.off("seat-lock-success",handleLockSuccess);
            socket.off("seat-locked-failed",handleLockFailed);
        };

    },[showId,data?.show?.location,selectedSeats,totalPrice,navigate]);
    ...
    const groupedSeats =
        data?.show?.seatLayout?.reduce((acc, row) => {
                if (!acc[row.type]) {
                    acc[row.type] = [];
                }
    ...
    const handleSeatToggle = (rowData,seat)=>{
        ...
        const seatIdStr=`${rowData.row}-${seat.number}`;
        if (lockedSeats.includes(seatIdStr)) {
            return;
        }
        setSelectedSeats((prev) => {
            const exists = prev.some((s) =>s.row === rowData.row &&s.number === seat.number);
            if (exists) {
                return prev.filter((s) =>!(s.row === rowData.row &&s.number === seat.number));
            }
            return [...prev,{row: rowData.row,number: seat.number,type: rowData.type,price: rowData.price}];
        });
    };
    ...
    const handleProceed = () => {
        ...
        const formattedSeatIds =selectedSeats.map((seat) =>`${seat.row}-${seat.number}`);

        setIsLocking(true);
        socket.emit("lock-seats", {showId:data?.show?._id ||showId,seatIds:formattedSeatIds,userId:user._id});
    };
                    ...
                    {Object.entries(groupedSeats || {}).map(([type, rows]) => (
                        <div
                            ...
                            <div className="flex flex-col gap-3 items-center">
                                {rows.sort((a, b) =>a.row.localeCompare(b.row)).map((rowData,idx) => (
                                            <div
                                                key={idx}
                                                ...
                                                <div className="flex gap-2 flex-wrap">
                                                    {rowData.seats.map((seat,i) => {
                                                            ...
                                                            const isLocked =lockedSeats.includes(seatIdStr);

                                                            const isOccupied = seat.status === "BOOKED" ||seat.status === "BLOCKED" ||isLocked;
                                                            return (
                                                                <div
                                                                    key={i}
                                                                    onClick={() =>
                                                                        handleSeatToggle(rowData,seat)
                                                                    }
